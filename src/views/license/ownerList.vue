<template>
<div class="mx-sm-5 mb-6 mt-3">
    <span class="font-weight-black">證照名稱：</span>
    {{ licenseName }}
                            ...
                            <v-card-text class="px-6 py-4">
                                <v-row>
                                    ...
                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field
                                            label="職務名稱"
                                            v-model.trim="editedItem.jobTitle"
                                        ></v-text-field>
                                    </v-col>
                                    ...
                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field
                                            label="證照生效日期"
                                            v-model.trim="editedItem.effectiveDate"
                                        ></v-text-field>
                                    </v-col>
                                    ...
                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field
                                            label="回訓日期"
                                            v-model.trim="editedItem.backTrainDate"
                                        ></v-text-field>
                                    </v-col>
                                </v-row>
                            </v-card-text>
...
export default {
    data: () => ({
        ...
        search: '',  // 搜尋的關鍵字
        headers: [  // 表格標題
            { text: '服務單位', value: 'unit', align: 'center', class: 'subtitle-1 black--text font-weight-bold' },
            { text: '姓名', value: 'name', align: 'center', class: 'subtitle-1 black--text font-weight-bold' },
            { text: '職務名稱', value: 'jobTitle', align: 'center', class: 'subtitle-1 black--text font-weight-bold' },
            { text: '證照編號', value: 'number', align: 'center', class: 'subtitle-1 black--text font-weight-bold' },
            { text: '證照生效日期', value: 'effectiveDate', align: 'center', class: 'subtitle-1 black--text font-weight-bold' },
            { text: '回訓日期', value: 'backTrainDate', align: 'center', class: 'subtitle-1 black--text font-weight-bold' },
        ...
        defaultItem: {
            ...
            jobTitle: '',  // 職務名稱
            number: '',  // 證照編號
            effectiveDate: '',  // 證照生效日期 (有值是不需要，所以用純字串手填)
            backTrainDate: '',  // 回訓日期 (有值是不需要，所以用純字串手填)
        },
    }),
    watch: {
        // dialog 遮罩照關閉時，表單回覆預設值
        dialogShow (val) {
            val || this.close()
        },
        // 路由參數變化時，重新向後端取資料
                ...
                this.tableItems = [
                    {
                        ...
                        jobTitle: '技正',
                        number: '061-022437',
                        effectiveDate: '103.09.20',
                        backTrainDate: '108.04.12',
                    },
                    {
                        unit: '阿里山車庫',
                        name: '許育任',
