...
import { MTBF } from '@/apis/materialManage/analy'
      ...
      if(parseInt(this.searchIpt.StartDay.replace(/-/g,"")) <= parseInt(this.searchIpt.EndDay.replace(/-/g,"")) ){
        ...
        MTBF({
          ...
          if (res.data.ErrorCode == 0) {
            ...
            let MTBFList = decodeObject(res.data.MTBFDataList)
            this.chartdata.labels.forEach(e=>{
              let eArr = e.split('/')
              let eYear = eArr[0]
              let eMonth = eArr[1]
              const MTBFFind = MTBFList.find(el=>(parseInt(el.Year)==parseInt(eYear)&&parseInt(el.Month)==parseInt(eMonth)))
              this.chartdata.datasets[0].data.push(MTBFFind?MTBFFind.MTBF:"0")
            })
          } else {
