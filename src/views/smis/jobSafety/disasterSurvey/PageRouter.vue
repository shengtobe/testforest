        ...
        fetchData() {
            ...
            setTimeout(() => {
                ...
                if (sessionStorage.getItem('itemStatus') !== null) {
                    ...
                    // 設定下面的欄位資料
                    let bottomItems = [
                        { dataType: 'text', oneline: true, icon: 'none', title: '本項工作經驗年數', text: obj.workYear },
                        { dataType: 'text', oneline: true, icon: 'none', title: '本項工作訓練日期', text: obj.trainingDate },
                        { dataType: 'text', oneline: true, icon: 'none', title: '罹災者住址', text: obj.address },
                        { dataType: 'text', oneline: true, icon: 'none', title: '發生單位', text: obj.depart },
                        { dataType: 'text', oneline: true, icon: 'none', title: '發生日期', text: `${obj.findDate} ${obj.findHour}:${obj.findMin}:00` },
                        { dataType: 'text', oneline: true, icon: 'none', title: '發生地點', text: obj.location },
                        { dataType: 'text', oneline: true, icon: 'none', title: '發生地點(緯度)', text: obj.lat },
                        { dataType: 'text', oneline: true, icon: 'none', title: '發生地點(經度)', text: obj.lng },
                        { dataType: 'text', oneline: true, icon: 'none', title: '氣候', text: obj.weather },
                        { dataType: 'text', oneline: true, icon: 'none', title: '事故類別', text: obj.accidentType },
                        { dataType: 'text', oneline: true, icon: 'none', title: '事故結果', text: obj.accidentResult },
                        { dataType: 'text', oneline: true, icon: 'none', title: '傷害部位', text: injurySiteOpts.find(item => item.value == obj.injurySite).text },
                        { dataType: 'text', oneline: true, icon: 'none', title: '災害類型', text: disasterTypeOpts.find(item => item.value == obj.disasterType).text },
                        { dataType: 'text', oneline: true, icon: 'none', title: '致傷媒介物', text: vehicleOpts[obj.vehicleLv1].find(item => item.value == obj.vehicle).text },
                        { dataType: 'text', oneline: false, icon: 'none', title: '直接原因', text: obj.directReason.replace(/\n/g, '<br>') },
                        { dataType: 'text', oneline: false, icon: 'none', title: '間接原因', text: obj.indirectReason.replace(/\n/g, '<br>') },
                        { dataType: 'text', oneline: false, icon: 'none', title: '基本原因', text: obj.basicReason.replace(/\n/g, '<br>') },
                        { dataType: 'text', oneline: false, icon: 'none', title: '傷者當時工作', text: obj.workItem.replace(/\n/g, '<br>') },
                        { dataType: 'text', oneline: false, icon: 'none', title: '事故概況', text: obj.overview.replace(/\n/g, '<br>') },
                        { dataType: 'text', oneline: false, icon: 'none', title: '緊急處理情形', text: obj.emergentWork.replace(/\n/g, '<br>') },
                        { dataType: 'text', oneline: false, icon: 'none', title: '事故單位防範及改善對策', text: obj.improveStrategy.replace(/\n/g, '<br>') },
                    ]
