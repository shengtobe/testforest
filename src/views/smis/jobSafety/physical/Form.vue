...
<v-container style="max-width: 1200px" class="label-header">
    ...
    <v-row>
      ...
      <!-- <v-col cols="12" sm="3" md="3">
        ...
        <peopleSelect :key="componentKey" :solo="true" valueCol="UserName" :peopleList="ipt.name" :disableList="[userData.UserName]" @getPeople="getPeople" />
      </v-col> -->
    </v-row>
    <v-form
        ref="form"
        ...
        <v-row class="px-2">
            ...
            <v-col cols="12" sm="4" md="3">
                ...
                <v-select
                    :rules="rules.Xray"
                    v-model="ipt.Xray"
                    :items="[{ text: '未選擇', value: '' },{ text: '1級', value: '1' },{ text: '2級', value: '2' },{ text: '3級', value: '3' },{ text: '4級', value: '4' }]"
                    solo
                ></v-select>
            </v-col>
            ...
            <v-col cols="12" sm="4" md="3">
                ...
                <v-select
                    :rules="rules.HepatitisB"
                    v-model="ipt.HepatitisB"
                    :items="['未選擇','Positive', 'Negative']"
                    solo
                ></v-select>
            </v-col>
            <!-- 尿酸 -->
...
export default {
    ...
    data: () => ({
      ...
      rules:{
        ...
        BloodShrink: [ // 血壓收縮壓 規則
          (v) => !!v || "必填欄位",
          (v) => (v && /^\d+?$/.test(v)) || "只允許有效數值",
          (v) => (v && v > 0) || "必填欄位",
          (v) => (v && v <= 600) || "數值過大",
        ],
        BloodDiastole: [ // 血壓舒張壓 規則
          (v) => !!v || "必填欄位",
          (v) => (v && /^\d+?$/.test(v)) || "只允許有效數值",
          (v) => (v && v > 0) || "必填欄位",
          (v) => (v && v <= 600) || "數值過大",
        ...
        HepatitisB: [ // Positive 規則
          (v) => !!v || "必填欄位",
          (v) => (v && v != '未選擇') || "必填欄位",
        ],
        HDLValue: [ // HDL 規則
          (v) => !!v || "必填欄位",
          (v) => (v && /^\d+([.]{1}\d{1,2})?$/.test(v)) || "只允許有效數值",
          (v) => (v && v > 0) || "必填欄位",
          (v) => (v && v <= 800) || "數值錯誤",
        ],
        LDLValue: [ // LDL 規則
    ...
    computed: {
      ...
      CholesterolGet: function() {
        if(this.ipt.LDLValue != '' && this.ipt.HDLValue != '' && this.ipt.Triglyceride != ''
        && +this.ipt.LDLValue > 10 && +this.ipt.LDLValue < 800 
        && +this.ipt.HDLValue > 9 && +this.ipt.HDLValue < 800 
        && +this.ipt.Triglyceride > 10 && +this.ipt.Triglyceride < 800){
                return +this.ipt.LDLValue + +this.ipt.HDLValue + +(this.ipt.Triglyceride/5)
            }
            else{
              return '--'
            }
